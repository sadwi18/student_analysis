...
# Endpoint 4: Return average G3 grouped by study time
@app.get("/by-studytime")
def get_average_g3_by_studytime():

    # Group students by studytime and calculate average G3
    avg_g3 = (
        df.groupby("studytime", as_index=False)["G3"]
        .mean()
        .rename(columns={"G3": "average_g3"})
    )

    # Round the average grade to 2 decimal places
    avg_g3["average_g3"] = avg_g3["average_g3"].round(2)

    # Return the result as a list of JSON records
    return avg_g3.to_dict(orient="records")
